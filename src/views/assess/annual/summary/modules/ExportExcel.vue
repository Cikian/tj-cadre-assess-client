    ...
    <a-spin :spinning="loading">
      <a-form-model :label-col="labelCol" :wrapper-col="wrapperCol">
        <a-form-model-item label="年度">
          <a-select v-model="year" placeholder="请选择年度">
...
export default {
  ...
  methods: {
    ...
    handleOk() {
      ...
      this.$emit('ok')
      axios
        .post('/modules/assessAnnualSummary/DetailExport', { year: this.year }, { responseType: 'blob' })
        .then((res) => {
          if (res && res.headers) {
            let contentDisposition = res.headers['content-disposition']
            let fileName = contentDisposition ? contentDisposition.split('filename=')[1] : '纪检明细.xls'
