...
import { JeecgListMixin } from '@/mixins/JeecgListMixin'
 import AssessDemocraticEvaluationModal from '../assess/annual/democratic/modules/AssessDemocraticEvaluationModal'
 import { filterMultiDictText } from '@/components/dict/JDictSelectUtil'
 import '@/assets/less/TableExpand.less'
 import AssessAnnualFillForm from '../assess/annual/assess/modules/AssessAnnualFillForm.vue'
import TModal from '@/component/TModal.vue'
 import number from 'ant-design-vue/lib/statistic/Number'
import { deleteCurrentUser, getDemocraticItems, getDemocraticOptions, submitDemocratic } from '@/api/assessApis'
import { getAction } from '@/api/manage'
...
export default {
  ...
  methods: {
    ...
    openDemocraticFill(departId) {
      ...
      for (let i = 0; i < temp.length; i++) {
        ...
        console.log('处理后的数据')
        console.log(this.democraticFillFormList)
      }
    ...
    beforeOk() {
      ...
      if (this.dataSource.length === 1 && this.userRoles.indexOf('director_leader') === -1) {
        this.$confirm({
          title: '即将退出登录',
          content: h => <div style='color:red;font-size:18px'>点击确认后将提交测评结果，系统自动退出登录，并且无法再次登录。请确认是否继续提交？</div>,
          onOk() {
            that.handleOk()
          },
          onCancel() {
        ...
        that.$confirm({
          title: '请确认',
          content: h => <div style='color:red;font-size:18px'>点击确认后将提交测评结果，并且无法修改，请仔细核对。请确认是否继续提交？</div>,
          onOk() {
            that.handleOk()
          },
          onCancel() {
  ...
  submitDemocratic(this.democraticFillFormList).then(res => {
    ...
    ,
    radioChange(e, i, j) {
      console.log(this.democraticFillFormList[i].items[j])
    }
    ,
      ...
      getAction(this.url.list, params).then((res) => {
        if (res.success) {
          ...
          if (params.pageNo === '1') {
            this.dataSource = []
            for (let i = 0; i < res.result.records.length; i++) {
              console.log('departIds.indexOf(this.fillLists.length[i].departId) === -1')
              console.log(departIds.indexOf(res.result.records[i].departId) === -1)
              console.log(params)
              // if (res.result.records[i].type === "23") {
