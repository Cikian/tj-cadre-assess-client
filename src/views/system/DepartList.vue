    ...
    <a-col :md='12' :sm='24'>
      <a-card :bordered='false'>
        ...
        <a-row style='margin-left: 14px'>
          ...
          <a-button v-has='"admin"' title='删除多条数据' @click='batchDel' type='default'>批量删除</a-button>
...
<!--              </a-form-model-item>-->
              ...
              <a-form-model-item :labelCol='labelCol' :wrapperCol='wrapperCol' prop='departName' label='处室（单位）全称'>
                <a-input placeholder='请输入处室（单位）全称' v-model='model.departName' />
              </a-form-model-item>

              <a-form-model-item :labelCol='labelCol' :wrapperCol='wrapperCol' label='标准简称'>
                <a-input placeholder='请输入处室（单位）标准简称' v-model='model.alias' />
              </a-form-model-item>
              ...
              <!--                </template>-->
              <!--              </a-form-model-item>-->
              <a-form-model-item :labelCol='labelCol' :wrapperCol='wrapperCol' prop='departType' label='单位类型'
                                 v-if='!orgCategoryDisabled'>
                <a-radio-group v-model='model.departType' placeholder='请选择单位类型'>
                  <a-radio value='1'>
                    局机关各处室
                  </a-radio>
                  <a-radio value='2'>
              ...
              <!--              </a-form-model-item>-->
              <a-form-model-item  v-has='"admin"' :labelCol='labelCol' :wrapperCol='wrapperCol' label='排序'>
            ...
            <div class='anty-form-btn'>
              <a-button  v-has='"admin"' @click='emptyCurrForm' type='default' htmlType='button' icon='sync'>重置</a-button>
...
<!--        <a-tab-pane tab='权限' key='2' forceRender>-->
...
export default {
  ...
  data() {
    return {
      ...
      validatorRules: {
        ...
        departType: [{ required: true, message: '请选择机构类型!' }],
  ...
  methods: {
    ...
    //     });
    //   };
    //   const data = [...this.departTree];

    //   // Find dragObject and its parent
    //   let dragObj;
    // let dragParent;
    ...
    // // Find dropObject and its parent
    // let dropObj;
    // let dropParent;
    ...
    // // Check if drag and drop nodes are at the same level
    // if (dragParent !== dropParent) {
    //   console.log('Cannot drag between different levels');
    //   return;
    // }

    // // Remove dragObj from its original position
    // const dragIndex = dragParent.indexOf(dragObj);
...
console.log("原data",data)
      ...
      // 确保节点不会被拖入其他节点的子节点中或更深的层级
      if (info.dropToGap) {
        const dropIndex = dropParent.indexOf(dropObj)
        if (dropPosition === -1) {
          dropParent.splice(dropIndex, 0, dragObj) // 插入到目标节点之前})
          putAction('/common/updateDepartSort', data).then(res=>{
            if(res.success){
console.log(res)
            }})
          ...
          dropParent.splice(dropIndex + 1, 0, dragObj) // 插入到目标节点之后
          putAction('/common/updateDepartSort', data).then(res=>{
            if(res.success){
console.log(res)
            }})
        ...
        if (dropParent === dragParent) {
          console.log('不能拖入其他节点')
          // this.$message.error('不能拖入其他节点2233');
          this.$confirm({
            title: '消息提示',
            content: h => <div style='color:red;'>不能拖入其他节点</div>,
            onOk() {
              console.log('OK')
            },
            onCancel() {
          ...
          // 还原拖拽节点到原始位置
          dragParent.splice(dragIndex, 0, dragObj)
          return
        } else {
          console.log('不能拖入更深的层级')
          // 还原拖拽节点到原始位置
          dragParent.splice(dragIndex, 0, dragObj)
          return
        }
    ...
    setValuesToForm(record) {
      console.log('单位类型：' + record.departType)
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          if (!this.currSelected.id) {
            ...
            this.$confirm({
              title: '消息提示',
              content: h => <div style='color:red;'>请点击选择要修改部门</div>,
              onOk: () => {
                console.log('OK')
              },
              onCancel: () => {
          ...
          httpAction(this.url.edit, this.currSelected, 'put').then((res) => {
            if (res.success) {
              ...
              this.$confirm({
                title: '消息提示',
                content: h => <div style='color:red;'>保存成功!</div>,
                onOk: () => {
                  console.log('OK')
                },
                onCancel: () => {
