...
export default {
  ...
  data() {
    return {
      uploadAction: window._CONFIG['domianURL'] + '/common/uploadDuty?type=' + this.type,
      headers: {},
      fileList: [],
      newFileList: [],
      uploadGoOn: true,
  ...
  methods: {
    ...
    initFileList(paths) {
      ...
      this.fileList = fileList
      console.log('jeuploadfilelist', this.fileList)
    },
    handlePathChange() {
    ...
    doBeforeUpload(file) {
      if (this.allowType){
        ...
        if (this.allowType.indexOf(fileExtension) < 0) {
          this.$error({
            title: '请上传' + this.allowType.join('、') + '格式的文件',
      ...
      console.log('jeuploadfile', file)
      if (this.deorper === 'de' || this.deorper === 'per') {
        if (this.logal !== file.name.split('.').shift()) {
          // return false
        }
      }
