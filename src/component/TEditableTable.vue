              ...
              <template slot="content">
                <a-form-model layout="horizontal" :labelCol="{span:8}" :wrapperCol="{span:16}">
                  ...
                  <a-form-model-item label="添加位置">
                    ...
                    <p style="font-size: 16px;color:#aaa;line-height: 14px;text-align: right;margin: 0;">0 = 最底部</p>
                  </a-form-model-item>
...
<!--          </div>-->
          ...
          <template v-for="col in columns">
            <div
              ...
              <span style='font-size:18px'>{{ col.title }}</span>
            </div>
          </template>
        </div>
      </div>
...
<!--                <div v-if="rowSelection" class="td td-cb" :style="style.tdLeft">-->
<!--                  &lt;!&ndash; 此 v-for 只是为了拼接 id 字符串 &ndash;&gt;-->
...
<!--                </div>-->
                ...
                <span v-if="row.mustFill" style="color: red; width: 10px">*</span>
...
export default {
  ...
  methods: {
    ...
    _pushByDataSource(dataSource, insertIndexes = null, update = true, rows = null, setDefaultValue = false) {
      ...
      let cascaderValues = { ...this.cascaderValues }
      ...
      dataSource.forEach((data, newValueIndex) => {
        ...
        let row = { id: dataId, canEdit: data.canEdit, mustFill: data.mustFill }
        let value = { id: dataId }
        let disabled = false
        this.columns.forEach(column => {
          let inputId = column.key + value.id
          let sourceValue
          if (data[column.key] instanceof Array) {
            sourceValue = data[column.key]
          } else {
          ...
          if (column.type === FormTypes.inputNumber) {
            ...
            if (!sourceValueIsEmpty) {
              ...
              if (sourceValue instanceof Array) {
                sourceValue = sourceValue.map(item => {
      ...
      console.log(this.selectValues)
      this.cascaderValues = cascaderValues
    ...
    removeSelectedRows() {
      ...
      this.beforeDelete().then(()=>{
        ...
        this.$confirm({
            ...
            content: h => <div style="color:red;">{e}</div>,
    ...
    _getAllValuesForSlot() {
      return cloneObject({
        ...
        cascaderValues: this.cascaderValues,
    ...
    handleChangeSelectCommon(value, id, row, column) {
      ...
      console.log("selected", this.selectValues)
      // 移除焦点
    },
    ...
    checkTooFastClick(key = 'default', ms = 300) {
      ...
      if (diffTime <= ms) {
        ...
        this.$confirm({
            ...
            content: h => <div style="color:red;">你点击的太快了，请慢点点击</div>,
