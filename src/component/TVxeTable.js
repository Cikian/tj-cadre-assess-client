...
import VxeWebSocketMixins from '@/components/jeecg/JVxeTable/mixins/vxe.web.socket.mixins'
import { initDictOptions } from '@/components/dict/JDictSelectUtil'

import { getRefPromise } from '@/components/jeecg/JVxeTable/utils/vxeUtils'
import { getEnhancedMixins, replaceProps } from '@/components/jeecg/JVxeTable/utils/cellUtils'

import JVxeToolbar from '@/components/jeecg/JVxeTable/components/JVxeToolbar'
import JVxeSubPopover from '@/components/jeecg/JVxeTable/components/JVxeSubPopover'
import JVxeDetailsModal from '@/components/jeecg/JVxeTable/components/JVxeDetailsModal'
import JVxePagination from '@/components/jeecg/JVxeTable/components/JVxePagination'
...
export default {
  ...
  methods: {
    ...
    blurAll() {
      // 1. 获取vxe-table实例
      const vxeGrid = this.$refs.vxe;
      if (vxeGrid) {
        const xTable = vxeGrid.$refs.xTable;

        // 2. 清除当前激活的单元格
        if (xTable && xTable.clearActived) {
          xTable.clearActived();
        }

        // 3. 强制关闭所有下拉框
        const dropdowns = document.querySelectorAll('.vxe-select--panel');
        dropdowns.forEach(dropdown => {
          dropdown.style.display = 'none';
        });

        // 4. 触发自定义失焦事件
        this.trigger('all-blur');
      }
    },
